...
/**
 * Tests the ability to generate training data for a 4-D TensorFlow neural-net model for slope estimation.
 * 
 * @author tgreen
 *
 */
public class GenEstimationTests_4d_a {
	...
	public static void main(String[] args) throws Throwable {
		...
		final FileOutputStream[][][][] foy = new FileOutputStream[ MAX_T ][ MAX_X ][ MAX_Y ][ MAX_Z ];
		...
		for( int t = 0 ; t < MAX_T ; t++ )
		{
			for( int x = 0 ; x < MAX_X ; x++ )
			{
				for( int y = 0 ; y < MAX_Y ; y++ )
				{
					...
					{	
						foy[ t ][ x ][ y ][ z ] = new FileOutputStream( "yvals_" + t + "_" + x + "_" + y + "_" + z + "_.txt" );
		...
		final PrintStream[][][][] psy = new PrintStream[ MAX_T ][ MAX_X ][ MAX_Y ][ MAX_Z ];
		...
		for( int t = 0 ; t < MAX_T ; t++ )
		{
			for( int x = 0 ; x < MAX_X ; x++ )
			{
				for( int y = 0 ; y < MAX_Y ; y++ )
				{
					for( int z = 0 ; z < MAX_Z ; z++ )
					{	
						psy[ t ][ x ][ y ][ z ] = new PrintStream( foy[ t ][ x ][ y ][ z ] );
	...
	/**
	 * The maximum inverse wave number along the Z-axis.
	 */
	static final int MAXWAV_Z = 5;
		...
		for( int t = 0 ; t < ( MAX_T - 1 ) ; t++ )
		{
			for( int x = 0 ; x < MAX_X ; x++ )
			{
				for( int y = 0 ; y < MAX_Y ; y++ )
				{
					for( int z = 0 ; z < MAX_Z ; z++ )
					{
						max = Math.max( max , Math.abs( iv[ t ][ x ][ y ][ z ] ) );
					}
				}
			}
		}
	...
	protected static void genCase( final int seed , final PrintStream psx , final PrintStream[][][][] psy  )
	{
		...
		final double[][][][] iv = new double[ MAX_T ][ MAX_X ][ MAX_Y ][ MAX_Z ];
		
		final double[][][][] ov = new double[ MAX_T ][ MAX_X ][ MAX_Y ][ MAX_Z ];
		...
		final double zsl = 300.0 * ( rand.nextDouble() - 0.5 );
		
		
		
		
		
		final double[][][][] mag = new double[ MAXWAV_T ][ MAXWAV_X ][ MAXWAV_Y ][ MAXWAV_Z ];
		
		final double[][][][] phase = new double[ MAXWAV_T ][ MAXWAV_X ][ MAXWAV_Y ][ MAXWAV_Z ];
		...
		for( int t = 0 ; t < MAX_T ; t++ )
		{
			for( int x = 0 ; x < MAX_X ; x++ )
			{
				for( int y = 0 ; y < MAX_Y ; y++ )
				{
					for( int z = 0 ; z < MAX_Z ; z++ )
					{	
						mag[ t ][ x ][ y ][ z ] = 2.0 * rand.nextDouble();
								
						phase[ t ][ x ][ y ][ z ] = rand.nextDouble();
					}
				}
			}
		}
			...
			for( int x = 0 ; x < MAX_X ; x++ )
			{
				for( int y = 0 ; y < MAX_Y ; y++ )
				{
					for( int z = 0 ; z < MAX_Z ; z++ )
					{
						...
						for( int t2 = 0 ; t2 < MAXWAV_T ; t2++ )
						{
							...
							for( int x2 = 0 ; x2 < MAXWAV_X ; x2++ )
							{
								...
								for( int y2 = 0 ; y2 < MAXWAV_Y ; y2++ )
								{
									...
									for( int z2 = 0 ; z2 < MAXWAV_Z ; z2++ )
									{	
										final double wz2 = z2 == 0 ? 0.0 : z / z2;
										
										wsum += calcSin( mag[ t ][ x ][ y ][ z ] , wt2 + wx2 + wy2 + wz2 , phase[ t ][ x ][ y ][ z ] );
						...
						iv[ t ][ x ][ y ][ z ] = startConst + tsl * t + xsl * x + ysl * y + zsl * z + wsum + randNoiseVal;
								
						ov[ t ][ x ][ y ][ z ] = startConst + tsl * t + xsl * x + ysl * y + zsl * z + wsum;
					}
				}
			}
		}
